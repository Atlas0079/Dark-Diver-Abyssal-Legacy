...
func init_from_data(skill_id: String, original_data: Dictionary, skill_config: Dictionary) -> void:
	# 创建原始数据的深度复制
	...
	self.priority = skill_config.get("priority", 0)
	...
	self.timings = skill_config.get("timings", [])
