...
# 初始化技能
	if data.battle.has("active_skills"):
		for skill_id in data.battle.active_skills:
			var skill_data = data.battle.active_skills[skill_id]
			var custom_timings = skill_data.get("timings", [])
			var custom_priority = skill_data.get("priority", -1)
			
			var skill = SkillRegistry.create_skill(skill_id, custom_timings, custom_priority)
			if skill:
				skill.owner = self
...
func modify_battle_stats(stat_name: String, amount: int) -> void:
	if battle_stats.has(stat_name):
		battle_stats[stat_name] += amount
	else:
		push_error("修改战斗属性失败，属性 %s 不存在" % stat_name)
...
func get_available_active_skill(battle: Battle) -> BaseSkill:
	# 创建一个包含技能ID和优先级的数组
	...
	for skill_data in prioritized_skills:
		...
		if skill != null and skill.get_targets(self,battle).size() > 0:
			return skill
	
	return null  # 如果没有可用技能则返回空字符串
...
func get_battle_stat(stat_name: String) -> int:
	var value = battle_stats.get(stat_name, 0)
	return StateManager.apply_state_effects_to_battle_stat(self, stat_name, value)
